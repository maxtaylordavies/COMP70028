...
class GridWorld(object):
        ...
        for action in range(4):
            for effect in range(4):
                ...
                for prior_state in range(S):
                    ...
                    T[post_state, prior_state,
                      action] = T[post_state, prior_state, action] + prob
    ...
    def is_location(self, loc):
        ...
        if (loc[0] < 0 or loc[1] < 0 or loc[0] > self.shape[0] - 1
    ...
    def generate_episode(self, policy, random_start=True, starting_loc=None):
        ...
        loc = self.locs[int(np.random.randint(len(
            self.locs)))] if random_start else starting_loc
        ...
        while loc not in self.absorbing_locs:
            ...
            outcome_idx = np.random.choice(
                [action_idx] +
                [i for i in range(len(action_probs)) if i != action_idx],
                p=self.action_randomizing_array)
            ...
            if next_loc in self.absorbing_locs:
                reward = self.special_rewards[self.absorbing_locs.index(
                    next_loc)]
            else:
    ...
    def compute_forward_discounted_return(self, episode, start, gamma):
        return sum([(gamma**i) * reward
                    for i, (_, _, reward) in enumerate(episode[start:])])
    ...
    def compute_backwards_discounted_return(self, episode, start, gamma):
        ...
        return sum([(gamma**i) * reward
                    for i, (_, _, reward) in enumerate(episode[start:][::-1])])
...
####### Methods #########
    ...
    def draw_deterministic_policy(self, Policy, p, gamma, new_figure):
        ...
        for state, action in enumerate(Policy):
            if (self.absorbing[0, state]):
                continue
            arrows = [
                r"$\uparrow$", r"$\rightarrow$", r"$\downarrow$",
                r"$\leftarrow$"
            ]
            ...
            for state_idx in range(policy.shape[0]):
                ...
                for action_idx in range(policy.shape[1]):
                    ...
                    for state_idx_prime in range(policy.shape[0]):
                        tmpQ = tmpQ + T[
                            state_idx_prime, state_idx, action_idx] * (
    ...
    def policy_iteration(self, policy, threshold, discount):
        ...
        while not policy_stable:
            ...
            for state_idx in range(policy.shape[0]):
                ...
                for action_idx in range(policy.shape[1]):
                    for state_idx_prime in range(policy.shape[0]):
                        action_values[action_idx] += T[
                            state_idx_prime, state_idx, action_idx] * (
                                R[state_idx_prime, state_idx, action_idx] +
                                discount * value[state_idx_prime])
                # and thus find the action that yields the highest value for this state
                best_action_idx = np.argmax(action_values)
                policy[state_idx] = np.eye(1,
                                           M=policy.shape[1],
                                           k=best_action_idx)
    ...
    def monte_carlo_iterative_optimisation(self,
                                           n,
                                           alpha,
                                           epsilon,
                                           gamma,
        ...
        for itr in iterable:
            ...
            for step_idx, (state_idx, action_idx, _) in enumerate(episode):
                if discounting == "forward":
                    R = self.compute_forward_discounted_return(
                        episode, step_idx, gamma)
                elif discounting == "backward":
                    R = self.compute_backwards_discounted_return(
                        episode, step_idx, gamma)
                else:
                    R = self.compute_undiscounted_return(
                        episode, step_idx, gamma)

                # update our Q-value for the pair in the direction of our experienced return
                Q[state_idx,
                  action_idx] += alpha * (R - Q[state_idx, action_idx])
            ...
            for state_idx in set([state_idx for (state_idx, _, _) in episode]):
                ...
                policy[state_idx] = [
                    1 - epsilon + (epsilon / self.action_size) if action_idx
                    == best_action_idx else epsilon / self.action_size
        ...
        for state_idx in set([state_idx for (state_idx, _, _) in episode]):
            best_action_idx = np.argmax(Q[state_idx])
            policy[state_idx] = [
                1 if action_idx == best_action_idx else 0
                for action_idx in range(self.action_size)
            ]
    ...
    def plot_learning_curve(self, returns_matrix, epsilon, alpha, new_figure,
                            show_error, color, algo_name):
        if new_figure:
            plt.figure()
        ...
        if show_error:
            plt.fill_between(np.arange(len(returns_average)),
                             returns_average - returns_std,
                             returns_average + returns_std,
                             alpha=0.2)
        ...
        plt.title(
            rf"{algo_name} learning curve for $\epsilon$={epsilon}, $\alpha$={alpha}"
        )
...
def q2b():
    p, gamma, threshold = 0.75, 0.3, 0.01
    grid = GridWorld(p)

    pol, val = find_optimal_policy_and_values(grid, p, gamma, threshold)
    grid.draw_deterministic_policy(format_deterministic_policy(pol),
                                   p,
                                   gamma,
                                   new_figure=True)
    grid.draw_grid_values(val, p, gamma, new_figure=True)
    ...
    for i, p in enumerate([0.1, 0.25]):
        ...
        plt.axis("off")
        grid.draw_deterministic_policy(format_deterministic_policy(pol), p,
                                       gamma, False)
...
def q2c():
    ...
    for run_idx in tqdm(range(num_runs)):
        policy, Q, returns, _ = grid.monte_carlo_iterative_optimisation(
            num_episodes, alpha, epsilon, gamma, discounting="backward")
    ...
    # plot learning curve
    grid.plot_learning_curve(returns_matrix,
                             epsilon,
                             alpha,
                             new_figure=True,
                             show_error=True,
    ...
    for i in range(len(epsilons)):
        ...
        returns_matrix = np.zeros((num_runs, num_episodes))
        for run_idx in tqdm(range(num_runs)):
            _, _, returns, _ = grid.monte_carlo_iterative_optimisation(
                num_episodes,
                alpha,
                epsilons[i],
                gamma,
    ...
    plt.legend((rf"$\epsilon=0$", rf"$\epsilon=0.1$", rf"$\epsilon=0.5$",
                rf"$\epsilon=1$"),
               loc="upper right")
    plt.show()
    ...
    plt.legend((rf"$\alpha=0.001$", rf"$\alpha=0.01$", rf"$\alpha=0.1$",
                rf"$\alpha=0.5$"),
               loc="upper right")
    plt.show()
...
def q2d():
    ...
    returns_matrix = np.zeros((num_runs, num_episodes))
    for run_idx in tqdm(range(num_runs)):
        _, Q, returns, _ = grid.sarsa(num_episodes,
                                      alpha,
                                      epsilon,
                                      gamma,
                                      discounting="backward",
    ...
    for i in range(len(epsilons)):
        ...
        returns_matrix = np.zeros((num_runs, num_episodes))
        for run_idx in tqdm(range(num_runs)):
            _, _, returns, _ = grid.sarsa(num_episodes,
                                          alpha,
                                          epsilons[i],
                                          gamma,
                                          discounting="backward",
        ...
        grid.plot_learning_curve(returns_matrix,
                                 ...
                                 algo_name="Temporal Difference")
    plt.title(
        r"Effect of varying $\epsilon$ on Temporal Difference learning curves")
    plt.legend((rf"$\epsilon=0$", rf"$\epsilon=0.1$", rf"$\epsilon=0.5$",
                rf"$\epsilon=1$"),
               loc="upper right")
    plt.show()
    ...
    for i in range(len(alphas)):
        ...
        grid.plot_learning_curve(returns_matrix,
                                 ...
                                 algo_name="Temporal Difference")
    plt.title(
        r"Effect of varying $\alpha$ on Temporal Difference learning curves")
    plt.legend((rf"$\alpha=0.001$", rf"$\alpha=0.01$", rf"$\alpha=0.1$",
                rf"$\alpha=0.5$"),
               loc="upper right")
    plt.show()
...
def q2e():
    # initialise parameters and construct gridworld
    num_runs, num_episodes, p, gamma, epsilon, alpha, threshold = 100, 500, 0.75, 0.3, 0.1, 0.005, 0.001
    grid = GridWorld(p)
    ...
    _, _, _, mc_errors = grid.monte_carlo_iterative_optimisation(
        num_episodes,
        alpha,
        epsilon,
        gamma,
        discounting="forward",
        analytical_values=dp_values)
    _, _, _, td_errors = grid.sarsa(num_episodes,
                                    alpha,
                                    epsilon,
                                    gamma,
                                    discounting="forward",
    ...
    plt.title(
        rf"Comparison of value RMSE curves for MC and TD ($\alpha={alpha}$)")
    plt.legend(("MC", "TD"))
    plt.show()
    ...
    for run_idx in tqdm(range(num_runs)):
        _, _, _, mc_error_matrix[
            run_idx] = grid.monte_carlo_iterative_optimisation(
                num_episodes,
                alpha,
                epsilon,
                gamma,
                ...
                show_progress=False,
                analytical_values=dp_values)
        _, _, _, td_error_matrix[run_idx] = grid.sarsa(
            num_episodes,
            alpha,
            epsilon,
            gamma,
    ...
    plt.fill_between(np.arange(len(mc_error_average)),
                     mc_error_average - mc_error_std,
                     mc_error_average + mc_error_std,
                     color="mediumseagreen",
                     alpha=0.2)

    plt.plot(td_error_average, color="darkblue")
    plt.fill_between(np.arange(len(td_error_average)),
                     td_error_average - td_error_std,
                     td_error_average + td_error_std,
                     color="darkblue",
                     alpha=0.2)
    ...
    _, _, mc_returns, mc_errors = grid.monte_carlo_iterative_optimisation(
        num_episodes,
        alpha,
        0,
        gamma,
        discounting="none",
        analytical_values=dp_values)
    _, _, td_returns, td_errors = grid.sarsa(num_episodes,
                                             alpha,
                                             0,
                                             gamma,
                                             discounting="none",
    ...
    plt.subplot(1, 2, 1)
    plt.scatter(mc_returns, mc_errors, s=1, color="mediumseagreen")
    plt.xlabel("Total discounted episode return")
    plt.ylabel(r"RMSE of $V^*$ function")
    plt.title("Monte-Carlo")

    plt.subplot(1, 2, 2)
    plt.scatter(td_returns, td_errors, s=1, color="darkblue")
    plt.xlabel("Total discounted episode return")
    plt.ylabel(r"RMSE of $V^*$ function")
    plt.title("Temporal Difference")

    plt.suptitle(
        rf"$V^*$ RMSE against total discounted episode return ($\alpha={alpha}$)"
    )
    plt.show()
